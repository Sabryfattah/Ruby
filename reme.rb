...
def self.save_task
open('c:/scripts/data/tasks.txt', 'a') {|f|
...
def self.get_tasks
@tasks = []
File.open('c:/scripts/data/tasks.txt').each do |line|
  @tasks.push(line)
end
end
...
choice = gets.chomp
  case choice 
   ...
   when 'd'
	...
	print_tasks
   when 'e'
     system"Notepad c:/scripts/data/tasks.txt"
    when 'q'
	clear
	return
  end
...
f = File.readlines("c:/scripts/data/tasks.txt")
n = x - 1
f.delete_at(n)
File.write('c:/scripts/data/tasks.txt', "", mode: 'w')
  f.each do |line|
    File.write('c:/scripts/data/tasks.txt', line, mode: 'a')
  end
end
